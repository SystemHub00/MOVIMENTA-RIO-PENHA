...
# =============================================================================
COURSE_CATALOG = [
    {"id": "1", "nome": "MONITOR DE LAZER E RECREA\u00c7\u00c3O"},
    {"id": "2", "nome": "GOVERNAN\u00c7A ESG"},
    {"id": "3", "nome": "AUXILIAR ADMINISTRATIVO"},
    {"id": "4", "nome": "ASSISTENTE DE LOG\u00cdSTICA"},
]
...
# =============================================================================
ADDRESS_OPTIONS = {
    "1": "\U0001f4cdRua Nossa Senhora Aparecida, n\u00b0 46, Penha - RJ (Programa Jos\u00fae Aranha)",
}
...
<head>
    ...
    <title>MOVIMENTA RIO — PENHA</title>
...
<body data-start-step="{{ current_step }}">
                    ...
                    <div class="step-card">
                        ...
                        <div class="review-layout">
                            ...
                            <div class="review-box full">
                                <div class="review-info-text" style="margin-bottom:10px; color:#1d4ed8; font-size:0.98rem; text-align:left;"><strong>Elegibilidade:</strong> Este curso &#233; destinado a pessoas com 16 anos ou mais interessadas em qualifica&#231;&#227;o profissional.</div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function setError(fieldId,msg){var field=document.getElementById(fieldId),err=document.getElementById(fieldId+'-error');if(field)field.classList.toggle('erro-campo',Boolean(msg));if(err){err.textContent=msg||'';err.hidden=!msg;}}
            ...
            courseSelect.addEventListener('change',function(){setError('curso_id','');var cursoId=courseSelect.value;if(cursoId&&optionByCursoId[cursoId]){aplicarOpcao(optionByCursoId[cursoId].id);}else{aplicarOpcao('');}syncReview();});
            if(btnCopiarEndereco&&enderecoInput){btnCopiarEndereco.addEventListener('click',function(){navigator.clipboard.writeText(enderecoInput.value).then(function(){btnCopiarEndereco.textContent='COPIADO \u2705';}).catch(function(){enderecoInput.select();document.execCommand('copy');btnCopiarEndereco.textContent='COPIADO \u2705';});setTimeout(function(){btnCopiarEndereco.textContent='COPIAR \U0001F4CB';},1200);});}
            ...
            function syncReview(){reviewTargets.forEach(function(target){var key=target.dataset.review;if(key==='curso_nome'){target.textContent=cursoInput?cursoInput.value.trim():'';return;}if(key==='local_nome'){target.textContent=localInput?localInput.value.trim():'';return;}var field=document.getElementById(key);if(!field){target.textContent='';return;}if(field.tagName==='SELECT'){var sel=field.options[field.selectedIndex];target.textContent=sel?sel.text.trim():'';}else{target.textContent=field.value.trim();}});}
            function validarCPF(cpf){var d=somenteDigitos(cpf);if(d.length!==11||/^(\d)\1+$/.test(d))return false;var soma=0,dig;for(var i=0;i<9;i++)soma+=Number(d[i])*(10-i);dig=(soma*10)%11;if(dig===10)dig=0;if(dig!==Number(d[9]))return false;soma=0;for(var i=0;i<10;i++)soma+=Number(d[i])*(11-i);dig=(soma*10)%11;if(dig===10)dig=0;return dig===Number(d[10]);}
            ...
            function validarPassoEscolher(){if(!courseSelect.value){setError('curso_id','Selecione um curso.');courseSelect.focus();return false;}if(!opcaoIdInput.value){setError('curso_id','N\u00e3o foi poss\u00edvel determinar a turma. Tente novamente.');return false;}return true;}
            ...
            async function buscarBairro(){var limpo=somenteDigitos(cepInput.value);if(limpo.length!==8)return;try{var res=await fetch('https://viacep.com.br/ws/'+limpo+'/json/');var data=await res.json();if(!data.erro&&data.bairro){bairroInput.value=data.bairro;vBairro();syncReview();}}catch(e){console.error(e);}}
            document.querySelectorAll('[data-next]').forEach(function(btn){btn.addEventListener('click',function(){var target=btn.dataset.next;if(target==='escolher'&&!validarPassoDados())return;if(target==='revisao'&&!validarPassoEscolher())return;syncReview();mostrarPasso(target);});});
            ...
            if(initCursoId){courseSelect.value=initCursoId;var op=initOpcaoId?courseOptionsById[initOpcaoId]:optionByCursoId[initCursoId];if(op)aplicarOpcao(op.id);}
...
def confirmacao():
    ...
    if not protocolo:
        return redirect(url_for("home"))
    home_url = "https://movimenta-rio-penha.onrender.com"
    return render_template_string(
